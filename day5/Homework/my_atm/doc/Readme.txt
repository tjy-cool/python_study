...
------------------------------------------------------
    同时，还实现了简单的购物车进行商品选购，
          然后登陆信用卡进行付款，
          并将购买商品记录在atm的用户信息日志中
        已有账号：user(passwd)，  tom(asdf) ,   jack(asdf)等

注：ATM的用户登陆和用户日志模块都用了装饰器，分别在my/atm/atm/core/login.py 和 my_atm/atm/core/logger.py文件中
    整个程序一般都可以按 q 结束整个程序，包括输入密码阶段
...
└── shopping    #购物车模块
    ...
    └── shop_core       #购物车核心模块
        ├── login.py               #登陆模块
        ├── main.py                #主程序模块
        └── user_manage.py         #用户管理模块，只做了界面，没有具体实现
